...
class AStar {
	constructor(gridWidth, gridHeight, tileSize, blockedRectangles, characterSize, edgeMargin = 2) {
		this.gridWidth = gridWidth; // Math.floor(gridWidth / tileSize);
		this.gridHeight = gridHeight; //Math.floor(gridHeight / tileSize);
		this.tileSize = tileSize;
		this.characterSize = characterSize;
		this.edgeMargin = edgeMargin;
		this.grid = Array.from({ length: this.gridWidth }, () => 
			Array(this.gridHeight).fill(0)
		);
		this.blockedRectangles = blockedRectangles;
		this.avoidEdges = true;
		this.initializeGrid();

		console.log(gridWidth, this.gridHeight);
	}
	...
	isWalkable(x, y) {
		...
		if (this.avoidEdges) {
			if (x < this.edgeMargin || x >= this.gridWidth - this.edgeMargin ||
