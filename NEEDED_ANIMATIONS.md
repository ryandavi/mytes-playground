...
| `metal_door` | map object | Missing | Yes | `32x96` vertical, `96x32` horizontal | 4 runtime facings | 5-frame open/close loop | Config points to `images/MapObjects/metal_door.png`, which is missing. |
...
| `slime`, `ghost`, `goblin` NPCs | map object | Missing / shared placeholder | Yes | `64x64` | 4 | `idle`, walk `N/S/E/W`, alert, chase | Only `npc_slime.gif` is referenced, and that file is missing. `ghost` and `goblin` also need distinct art instead of sharing slime data. |
