...
3. **The floor's edge sits on the centreline of its boundary cell** — masonry or not (§4.7). Ending a painted floor on the cell edge meant that walling its own perimeter took the boundary cell over and jumped the floor half a tile inward. `FloorOwnershipResolver.insetOpenBoundaries` pulls a plan's open edges in by one block before expansion, so a floor painted on cells 3–6 spans 3.5–6.5 whether or not it is walled — verified identical, 24 blocks either way. Two guards keep it from eating small plans: a side is pulled in only where the plan continues along that side's axis (a lone tile stays a whole tile, a one-cell corridor keeps its width), and the map edge is not open ground (or the whole outdoors gets a bare half-tile frame). Fixtures `l-room` and `open-corridor` re-expected; every walled fixture is unchanged, which is the check that the tuck still works. The property test's "a seed keeps all four blocks" became "a seed keeps at least one block and never yields one to another plan".
...
Measured in the running app: a painted floor's edge sits on its boundary cell, and drawing a free-standing wall stub on the next cell out moves it to that wall's centreline — **half a cell outward**, from expansion tucking the floor under masonry that is not enclosing anything. The plan does not grow (seeds 20 → 20), so `proposeSeeds` is not involved. It is also what puts stray part-tiles at a stub's rounded end caps: floor tucked into the end cells, showing past art that does not fill them.

**An enclosure gate was tried and rejected.** Gating the tuck on "the plan's cell lies in an open component that does not touch the map edge" broke ten fixtures: every room that runs to the edge of the map stops tucking under its own walls (`horizontal-divider`, `terminal-caps`, `double-wall`, `vertical-divider`, `courtyard`, `incomplete-divider`, `deleted-perimeter-wall`, `post-corner-contest`). "Separates two open components" fails too — in `red-line-split` you can walk around the wall, so it separates nothing, yet the tuck there is correct and reviewed.

So the distinguishing rule is **not** enclosure and **not** separation, and two attempts at stating it locally have failed against the fixture set. This needs design thought rather than another guess. The honest framing for whoever picks it up: a wall stub in the open and a wall that bounds a room sit on identical cells relative to the floor, so any rule must appeal to something neither local nor purely topological — most likely the *player's intent* recorded at edit time (was this wall drawn as part of a room's boundary?) rather than geometry recovered afterwards.
