...
19. **`@mixin inset-scroll-box`** is the panel-inside-a-panel every build list sits in: sunken, its own scrollbar, a ceiling rather than a height. `.panel-list`, the Navigator's site list and `.build-inspector-details` all use it; the window itself never scrolls.
