...
**An arm that ends has no sides where it ends.** Revised 2026-09-05. `paintSpansForCell` gave a cell posts whenever a vertical connected to it and the horizontal ran both ways — so a T where an arm merely *terminates* into a run was cut like a crossing, a 14px post taken out of the run's face and dressed in the arm's rooms. At that cell the arm's west and east faces are inside the run's own masonry; what the camera sees across the whole cell is the run. A corner (`NE`, `NW`, `ES`, `SW`) already presented as two bands for exactly this reason, and the T (`NEW`, `ESW`) now does too: the test is whether the vertical *passes through* (`inheritsVerticalFace`), not how many ways the horizontal goes. `NES`, `NSW` and `NESW` are unchanged — there the vertical does pass through, its sides are real, and `postSurface` still carries them across the junction. Pinned by `an arm hanging off a run starts below the junction, not at it` and `the junction cell is band, and each half joins the run on its own side`.
...
**Second pass, same day — build-mode UI**

8. **The Openings palette never changed.** It is a view of the inventory and was rendered once, when the Structure tool opened, so a door you had just hung was still listed at its old count. `Inventory.updateInventoryDisplay` — the one place every add, remove and quantity change lands — now emits `INVENTORY_CHANGED`, and the palette follows it.
9. **"No entrance" never cleared.** Two causes. `openingAdjacency` records room-to-room pairs, so a door to the outside — a front door — was not an entrance at all; and a room with no door of its own but an open boundary onto one that has one was called sealed. `RoomTopology` now also publishes `openingRooms` (roomId → the kinds of opening reaching it), and the badge asks whether this room *or any room sharing its enclosure* has a door.
10. **Two room-type vocabularies.** `plan.roomType` holds the authored `zoneType` the AI reads (`rest`, `play`, `food`, `social`), while `SiteConfig.rooms.types` offered `bedroom`, `kitchen`, `playroom` — so every authored room showed a blank type in both dropdowns, and picking one silently overwrote its zone. They are one list now: the `id` is the zone type, the `label` is the room kind a player recognises, and kinds with no zone counterpart (bathroom, study, hallway, storage) simply emit none. A dropdown that meets a value it does not know keeps it rather than blanking it (`BuildInspector.selectValue`).
11. **What a room still needs.** `SiteConfig.rooms.types[].requires` lists object types that make a room of that kind finished, `indoorRequires` what every indoor room wants (a light), and the Inspector reports what is missing on the selected room: `No entrance`, `No window`, `No bed`, `No seat`, `No toy`, `No storage`, `No light`. Reported, never enforced. Objects are counted where they stand — centre across, foot down — so carrying a bed out of a bedroom takes the badge back with it. The Navigator keeps only structural badges (Open, Disconnected, No entrance, Empty); this belongs to the room you are looking at.
12. **Buildings have a type**, the same authored-vocabulary-nothing-reads-yet as room types: House, Storage, Workshop, Shop, Greenhouse, Barn, Outbuilding.
13. **Actions that cannot run are disabled**, with the reason on hover, rather than answering with a message when pressed. Separate needs more than one connected component (`BuildMarqueeSelection.buildingComponents`), Merge needs walls from more than one building selected.
14. **The Navigator scrolls in its own inset box** (`.build-navigator-node--site > ul`, 280px, the same `panel-list` treatment the room and ground lists use). The panel itself neither scrolls nor clips.
15. **Picking a room in Room Areas looks at it.** `RoomPanel.revealRoom` eases the camera to the room's centre; a room with no floor yet has nowhere to look, so nothing happens.
16. **A building now travels with its contents.** Walls carried their own openings and fixtures because those are records in the document; furniture standing on the floor is not, so it stayed behind. `contentMove.objectCells` (the selected rooms' resolved cells) says what ground is travelling, `collectStandingObjects` finds what is standing on it, and — because the document does not hold those positions — `BuildTransaction.recordHistory` takes `sideEffects` so undo and redo move them back and forth with the walls. The same objects are added to the travelling set for validation: a bed by the west wall stood in the destination of that same west wall and refused the whole move, atomically, with no way to see why.

**Third pass, same day — tools and chrome**

17. **Select reads the wall you can see.** A wall's art leans back off its own cell, so a click on the art landed on the floor behind it. `wallCellAt` maps the pointer's world position into each piece's box and tests the regions the renderer published, frontmost first — geometric, so the pieces stay `pointer-events: none` and objects, floors and the marquee keep working. Clicks also escalate now: segment → run → building, one step per click, so a double click gives the run and a third the building. A cell already in the selection is still left alone on a single click (dragging a group must not reset it), but a second click is not a grab.
18. **Badges are a generic.** `@mixin badge` in `primitives/_ui.scss`, built on the existing `pill-label`, with the two rules a badge always wants and always forgot: it is the width of its own text, and its text never breaks. The needs row was a `.setting-hint`, which is a two-column grid for an icon and a paragraph — that is what stretched the second badge across the row.
19. **`@mixin inset-scroll-box`** is the panel-inside-a-panel every build list sits in: sunken, its own scrollbar, a ceiling rather than a height. `.panel-list` provides it to Room Areas, Ground layers, and every Build Inspector view; `.build-inspector-details` uses the same inset treatment for read-only values.
20. **Navigator rows are one line**, the label clipped with an ellipsis and the full name on hover, and nested lists indent enough to read as a tree.
21. **Legacy room types are renamed on the way in.** A save written between §13c.10 and it holds `bedroom`/`kitchen`/`living`/`playroom`, which no dropdown offers; `RoomPlanStore.LEGACY_TYPES` maps them to the zone ids they became, rather than keeping a "(custom)" value the player cannot pick again.
22. **A room's colour can be chosen.** `RoomPlan.colourIndex` indexes the same hue wheel the automatic colour picks from; null means automatic, which is still the default and still the answer for almost every room. `RoomPanel.chosenColours` mirrors the plans on every commit so the static `roomColour` — called from the floor overlays, the rows and the ghost tiles — keeps agreeing with the panel.

**Fourth pass, same day — the tools catch up**

23. **Duplicate takes the furniture with it.** The same `collectStandingObjects` the move uses finds what is standing in the rooms being copied, and the copies are minted after the walls land — they need somewhere to stand. Undo removes them and redo mints them again from the *same* history entry as the walls: `applyWallCellChanges` now forwards a caller's `sideEffects` and composes them with its own. Doors, windows and wall fixtures still stay with the original — cloning one means minting a new opening record bound to a new object through the placement path, which is its own piece of work.
24. **A wall selection is editable.** Construction (rebuilding those cells, carrying the connect group with it unless it was authored to something else) and height, applied to every selected cell through one transaction, over a details list of connect group, building and resolved finish, with Paint · Duplicate · Demolish beneath.
25. **Room Areas groups by building** — headings, and the building's name on every row — but only once a map has more than one building. A hierarchy over the only group there is is a row you read past to reach the list, and the Navigator is where the shape of the site belongs.
26. **Keyboard and gesture:** arrow keys nudge a selection one cell through the same transaction a drag uses, Escape abandons a structure drag mid-gesture (a new first layer in `handleEscape`), and the Inspector's **Move** now arms the drag rather than printing a hint — the next press anywhere inside the selection, floor included, takes hold of it.
27. **A building row says what it holds** (`5 rooms · 166 walls`), so a building with no walls is legible without clicking it.
28. **The Navigator no longer widens its panel.** Grid and flex children default to `min-content`, and a row's min-content is its longest unbreakable word plus its badge; the view and the inset box now hold themselves to the panel width and clip.

**Fifth pass — what a click on a wall means**

29. **The base is the structure, the face is the surface.** A wall's base — the cell it stands on — is the wall itself, and clicks there widen: segment → run → building → segment, cycling, because at the top the only useful answer to "more" is to start again. The face standing up off that cell covers the ground behind it, and a click there selects **that surface**, the way a click on a floor selects its room: `BuildSelection` goes to `{kind: 'atom'}`, the face is outlined, and both the stage bar and the Inspector offer Paint this · Paint the room · Select the wall. `SurfaceCustomizePanel.openWallSurface` hands one surface to the Paint tool whole. One tool, two questions, told apart by where you click.
30. **Wall height is a readout, not an editor.** `heightCells` does not decide how tall a wall stands — the construction's art does — it decides which cells merge into one run and one cutaway chain. An editor for it looked like a height control and behaved like a seam. The construction dropdown likewise disables itself while `wall-materials.json` defines only one construction, so it is obvious where a second would appear.
31. **Doors came home with their walls.** `WallRenderer.invalidate` refreshed the runtime opening and fixture records only on the non-geometry path, so undoing a move replayed the records back to where they were while the door, window and painting *objects* stayed at the far end. The geometry path now syncs from the document and rebinds them, which is the same rebind a placement does.

**Sixth pass — the click that never counted**

32. **Double click never widened anything.** `event.detail` is the multi-click count for *mouse* events and zero for the pointer events this handler receives, so `event.detail >= 2` was never true and the escalation could not fire however many times you clicked. `countClick` keeps its own count — same cell, within `gestures.doubleClickInterval` — and the ladder works from the map: segment → run → building → segment.
33. **A face selects the stretch, not the atom.** A half-cell atom is the unit paint is *stored* in, not the unit anyone points at: clicking a wall means "this piece of wall", and the piece runs as far as the surface does (`getPaintStretchSurfaces`, the same set Paint's Section colours, outlined the same way).
34. **Exterior faces get exterior scopes.** The old bar offered "Paint the room", disabled, on a face that belongs to no room. It now offers *Paint section · Paint outside · Paint the building* out there and *Paint section · Paint the room* inside, and `openWallSurface(surface, scope)` hands Paint the scope as well as the surface, so "paint the room" arrives ready to paint the room.
