...
class Particle {
    ...
    activate(system, config, spawnState = {}) {
        ...
        this.spriteFrames = config.spriteFrames ? config.spriteFrames.slice() : null;
        this.spriteFrameIndex = 0;
        this.spriteFrameElapsed = 0;
        this.spriteFrameDuration = config.frameDuration;
        this.spriteFrameWidth = config.spriteFrameWidth;
        this.spriteFrameHeight = config.spriteFrameHeight;
        this.spriteSheetColumns = 1;
        this.spriteSheetRows = 1;
        this.spriteLoop = config.loop !== false;

        if (this.spriteFrames && this.spriteFrames.length > 0) {
            const frameBounds = this.spriteFrames.reduce((bounds, frame) => ({
    ...
    advanceSprite(deltaTime) {
        if (!this.useSprite || !this.spriteFrames || this.spriteFrames.length <= 1) {
            return;
        }

        this.spriteFrameElapsed += deltaTime;
        while (this.spriteFrameElapsed >= this.spriteFrameDuration) {
            this.spriteFrameElapsed -= this.spriteFrameDuration;
            const nextFrame = this.spriteFrameIndex + 1;

            if (nextFrame >= this.spriteFrames.length) {
                if (!this.spriteLoop) {
                    this.spriteFrameIndex = this.spriteFrames.length - 1;
                    break;
                }
                this.spriteFrameIndex = 0;
            } else {
                this.spriteFrameIndex = nextFrame;
            }
        }
    }
    ...
    updateVisual(deltaTime, alpha, system) {
        ...
        if (this.useSprite && this.spriteFrames && this.spriteFrames.length > 0) {
            const [frameX, frameY] = this.spriteFrames[this.spriteFrameIndex] || [0, 0];
