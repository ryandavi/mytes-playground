class Container {
    ...
    ********************************************/
	...
	update(time) {
		...
		this.mytes.forEach(myte => {
			if (myte.isActive) {
				// Update myte and handle frame updates
				myte.update(time);
				if (time - this.lastFrameTime >= this.frameInterval) {
					myte.update_frame();
				}

				// Ensure that the myte is updated after setting the target
				myte.update(time);
			}
		});

		if (time - this.lastFrameTime >= this.frameInterval) {
			this.lastFrameTime = time; // Update last frame time for frame interval checks
		}
